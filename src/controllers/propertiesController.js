...
folder. */
const {
    getExtraPropertyImages: getExtraPropertyImagesQuery
...
exports.getExtraPropertyImages = (req, res) => {
    const { id } = req.params;
    const { errors, valid } = validateIdAsNumeric(id);
    if (!valid) {
        return Response.send(
        ...
        db.query(getPropertyByIdQuery, [
            ...
            if (result.length > 0) {
                db.query(getExtraPropertyImagesQuery, [
                    id
                ], function(err, result) {
                    if (result.length > 0) {
                        res.status(201).json({
                        ...
                        res.status(401).json({
                            status: 'error',
                            error: "Oops, there's no Extra Images for this Property",
                        });
                    }
                })
            } else {
